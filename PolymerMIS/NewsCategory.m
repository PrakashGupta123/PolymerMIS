...
@synthesize headerLabel, backButton, tableViewForCategories, buttonMenu;
...
- (void)viewDidLoad
{
   ...
   // [tableViewForCategories setSeparatorColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"bdr.png"]]];
...
-(void)requestFinishedForNews:(ASIHTTPRequest *)request
{
    ...
    else
    {
       ...
       // NsLog(@"%@",responseDictionary);
        ...
        else if ([str5 isEqualToString:@"Please Install new build !"])
        {
            ...
            UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"Alert" message:@"Please Install new build !" delegate:self cancelButtonTitle:@"Ok" otherButtonTitles: nil];
            alert.tag=501;
            [alert show];
...
//            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:urlStr]];
        ...
        if([[[[responseDictionary objectForKey:@"response"] objectForKey:@"newCategoryResponse"] objectForKey:@"Table"] isKindOfClass:[NSArray class]])
        {
            categoryArray=[[[responseDictionary objectForKey:@"response"] objectForKey:@"newCategoryResponse"] objectForKey:@"Table"];
        ...
        {
            [[[UIAlertView alloc] initWithTitle:@"Alert" message:[[[responseDictionary objectForKey:@"response"] objectForKey:@"newCategoryResponse"] objectForKey:@"error"] delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil] show];
        }
...
-(void)callWebService
{
    ...
    else
    {
      ...
      //  NSURL *url = [NSURL URLWithString:@"http://www.polymermis.com/webservice_test.php"];
        ...
        [request setPostValue:@"web_news_category" forKey:@"methodName"];
       
        [request setDelegate:self];
        [request setDidFinishSelector:@selector(requestFinishedForNews:)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NewsCategoryDetails *details=[[NewsCategoryDetails alloc] init];
    details.categoryID=[[categoryArray objectAtIndex:indexPath.row] objectForKey:@"category_id"];
    details.categoryName=[[categoryArray objectAtIndex:indexPath.row] objectForKey:@"category_name"];
...
{
    if ([tableView isEqual: _MenuTable]) {
        return [menuArray count];
    } if ([tableView isEqual: self.Menutableipad]) {
        return [menuArray count];
    }
    else{
...
cell.imagMenu.image=[UIImage imageNamed:[menuArrayimg objectAtIndex:indexPath.row] ];
    ...
    NewsCategoryCell *cell=[tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if(cell==nil)
    {
        cell=[[NewsCategoryCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
    ...
    [cell.textLabelForCategory setText:[[categoryArray objectAtIndex:indexPath.row]objectForKey:@"category_name"]];
